...
export const loginAdmin = async (request,response, next) => {
    try {
        ...
        if (!admin) return next(Problem(400, "You are not an admin or you are not have admin access"));
        
        const checkPassword = bcrypt.compareSync(request.body.password, admin.password);

        if (!checkPassword) return next(Problem(400, "Password is incorrect!"));
        
        const token = jwt.sign({ admin: admin._id }, process.env.SECRET, { expiresIn: "2h" });
